...
watchEffect(() => {
  ...
  root.style.setProperty('--color-primary-15', `rgba(${r},${g},${b},0.15)`)
...
const navGroups = computed(() => [
  ...
  { label: 'Ventas', items: [
    { name: 'Caja', href: `${basePath.value}/ventas`, icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' },
    { name: 'Gastos', href: `${basePath.value}/gastos`, icon: 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { name: 'Facturacion', href: `${basePath.value}/facturacion`, icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
    { name: 'Paquetes', href: `${basePath.value}/paquetes`, icon: 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7' },
  ]},
  ...
  { label: 'Gestion', items: [
    ...
    { name: 'Sucursales', href: `${basePath.value}/sucursales`, icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
...
const navigation = computed(() => navGroups.value.flatMap(g => g.items))
...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 z-50 transition-all duration-300 hidden lg:flex flex-col border-r border-gray-200 bg-white',
    ...
    >
      ...
      <div class="flex items-center justify-between h-14 px-4 border-b border-gray-100">
        <div v-if="sidebarOpen" class="min-w-0">
          <p class="text-[14px] font-semibold truncate" style="color: var(--color-primary);">{{ tenant?.name || 'Salon SaaS' }}</p>
          <p class="text-[11px] text-gray-400 -mt-0.5">Salon de belleza</p>
        </div>
        <button @click="sidebarOpen = !sidebarOpen" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition">
      ...
      <nav class="flex-1 px-3 py-3 overflow-y-auto">
        <div v-for="group in navGroups" :key="group.label" class="mb-4">
          <p v-if="sidebarOpen" class="px-3 mb-1.5" style="font-size:10px; font-weight:500; color:#AAAAAA; text-transform:uppercase; letter-spacing:0.08em;">{{ group.label }}</p>
          <div class="space-y-0.5">
            <Link
              v-for="item in group.items"
              :key="item.name"
              :href="item.href"
              class="sidebar-item flex items-center px-3 py-2 rounded-lg transition-colors border-l-[3px]"
              :class="$page.url.startsWith(item.href) ? 'sidebar-active' : 'sidebar-inactive'"
            >
              <svg class="w-[18px] h-[18px] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon" />
              </svg>
              <span v-if="sidebarOpen" class="ml-3 truncate" style="font-size:13px;">{{ item.name }}</span>
            </Link>
          </div>
        </div>
      </nav>
      ...
      <!-- Footer -->
      <div v-if="sidebarOpen" class="px-4 py-3 border-t border-gray-100">
        <p class="text-[10px] text-gray-400">Plan {{ tenant?.plan || 'Profesional' }}</p>
        <p class="text-[10px] text-gray-400">{{ user?.name }}</p>
      </div>
    </aside>
    ...
    <div :class="['transition-all duration-300', sidebarOpen ? 'lg:pl-64' : 'lg:pl-20']">
      <!-- Topbar -->
      <header class="sticky top-0 z-40 bg-white" style="border-bottom: 2px solid var(--color-primary);">
          ...
          <Sheet v-model:open="mobileOpen">
            ...
            <SheetContent side="left" class="w-64 p-0">
              <div class="flex items-center h-14 px-4 border-b border-gray-100">
                <div>
                  <p class="text-[14px] font-semibold" style="color: var(--color-primary);">{{ tenant?.name || 'Salon SaaS' }}</p>
                  <p class="text-[11px] text-gray-400 -mt-0.5">Salon de belleza</p>
                </div>
              </div>
              <nav class="px-3 py-3">
                <div v-for="group in navGroups" :key="group.label" class="mb-4">
                  <p class="px-3 mb-1.5" style="font-size:10px; font-weight:500; color:#AAAAAA; text-transform:uppercase; letter-spacing:0.08em;">{{ group.label }}</p>
                  <div class="space-y-0.5">
                    <Link
                      v-for="item in group.items"
                      :key="item.name"
                      ...
                      :class="$page.url.startsWith(item.href) ? 'sidebar-active' : 'sidebar-inactive'"
                    >
                      <svg class="w-[18px] h-[18px] mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon" />
                      </svg>
